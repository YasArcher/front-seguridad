...
class AES128 {
  ...
  private static readonly INV_SBOX: number[] = [
  ...
  private static subBytes(state: State): State {
    for (let r = 0; r < 4; r++) {
      for (let c = 0; c < 4; c++) {
  ...
  private static invSubBytes(state: State): State {
    for (let r = 0; r < 4; r++) {
      for (let c = 0; c < 4; c++) {
  ...
  private static shiftRows(state: State): State {
  ...
  private static invShiftRows(state: State): State {
  ...
  private static stateToBytes(state: State): Uint8Array {
    const bytes = new Uint8Array(16);
    for (let c = 0; c < 4; c++)
      for (let r = 0; r < 4; r++) bytes[c * 4 + r] = state[r][c];
    return bytes;
  }
  ...
  private static unpad(data: Uint8Array): Uint8Array {
    const padLen = data[data.length - 1];
    return data.slice(0, data.length - padLen);
  }

  private static normalizeKey(key: string): number[] {
    return key
      .padEnd(16, "\0")
      .slice(0, 16)
      .split("")
